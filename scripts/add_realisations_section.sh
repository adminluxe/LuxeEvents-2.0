...
echo "==> 1) Crée le composant RéalisationsSection"
mkdir -p src/components
backup src/components/RealisationsSection.jsx

cat > src/components/RealisationsSection.jsx <<'EOF'
...
echo "✅ RealisationsSection.jsx créé."

echo
echo "==> 2) Trouve le fichier qui compose la Home (Hero/Services/Témoignages)"
HOMEFILE="$(
  grep -RIl --exclude='*.bak*' --exclude-dir='node_modules' --exclude-dir='_backups' \
    -E '(<HeroSection\b)|(<ServicesSection\b)|(<Testimonials\b)|(<Temoignages\b)' \
    src/pages src/app src/components src/App.jsx 2>/dev/null \
  | head -n 1 || true
)"

if [ -z "${HOMEFILE:-}" ]; then
  echo "❌ Home file introuvable. Lance:"
  echo "   grep -RIn \"<HeroSection\" src | head -n 40"
  exit 1
fi
...
echo
echo "==> 3) Injecte import + place la section après Services"
# inject import si absent
if ! grep -q 'RealisationsSection' "$HOMEFILE"; then
  sed -i '1,40{/^import /{h;d};$!b;}' "$HOMEFILE" 2>/dev/null || true
  # ajoute import après la dernière ligne import...
  awk '
    BEGIN{done=0}
    /^import /{print; last=NR; next}
    {print}
  ' "$HOMEFILE" > "$HOMEFILE.tmp"

  # insertion simple: après le dernier import trouvé
  # (fallback sed si awk n’a pas position)
  if ! grep -q 'import RealisationsSection' "$HOMEFILE.tmp"; then
    # on injecte après la dernière ligne "import ..."
    perl -0777 -pe 's/((?:^import .*?\n)+)/$1import RealisationsSection from ".\/components\/RealisationsSection.jsx";\n/sm' -i "$HOMEFILE.tmp"
  fi
  mv "$HOMEFILE.tmp" "$HOMEFILE"
fi

# insert <RealisationsSection /> juste après le premier <ServicesSection .../> ou <Services .../>
if grep -q '<ServicesSection' "$HOMEFILE"; then
  perl -0777 -i -pe 's/(<ServicesSection\b[\s\S]*?\/>)/$1\n      <RealisationsSection \/>/m' "$HOMEFILE"
elif grep -q '<Services\b' "$HOMEFILE"; then
  perl -0777 -i -pe 's/(<Services\b[\s\S]*?\/>)/$1\n      <RealisationsSection \/>/m' "$HOMEFILE"
else
  echo "⚠️ Impossible de trouver <ServicesSection/>. Je place la section avant Témoignages."
  perl -0777 -i -pe 's/(<(TestimonialsSection|Testimonials|Temoignages)\b)/<RealisationsSection \/>\n      $1/m' "$HOMEFILE"
fi

echo "✅ Injection terminée."
echo
echo "==> 4) Vérif: id=\"realisations\" unique"
grep -RIn --exclude='*.bak*' --exclude-dir='node_modules' --exclude-dir='_backups' 'id="realisations"' src | sed -n '1,20p' || true
